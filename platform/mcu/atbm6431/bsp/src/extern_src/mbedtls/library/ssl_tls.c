...
FLASH_FUNC static void mbedtls_zeroize( void *v, size_t n ) {
    volatile unsigned char *p = v; while( n-- ) *p++ = 0;
}

/* Length of the "epoch" field in the record header */
FLASH_FUNC static inline size_t ssl_ep_len( const mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC static void ssl_set_timer( mbedtls_ssl_context *ssl, uint32_t millisecs )
{
 ...
 */
FLASH_FUNC static int ssl_check_timer( mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC static int ssl_double_retransmit_timeout( mbedtls_ssl_context *ssl )
{
...
FLASH_FUNC static void ssl_reset_retransmit_timeout( mbedtls_ssl_context *ssl )
{
...
FLASH_FUNC static int ssl_session_copy( mbedtls_ssl_session *dst, const mbedtls_ssl_session *src )
{
...
FLASH_FUNC static int ssl3_prf( const unsigned char *secret, size_t slen,
                     const char *label,
...
FLASH_FUNC static int tls1_prf( const unsigned char *secret, size_t slen,
                     const char *label,
                     const unsigned char *random, size_t rlen,
                     unsigned char *dstbuf, size_t dlen )
{
...
FLASH_FUNC static int tls_prf_generic( mbedtls_md_type_t md_type,
...
FLASH_FUNC static int tls_prf_sha256( const unsigned char *secret, size_t slen,
                           const char *label,
                           const unsigned char *random, size_t rlen,
                           unsigned char *dstbuf, size_t dlen )
{
...
FLASH_FUNC static int tls_prf_sha384( const unsigned char *secret, size_t slen,
                           const char *label,
                           const unsigned char *random, size_t rlen,
                           unsigned char *dstbuf, size_t dlen )
{
...
FLASH_FUNC int mbedtls_ssl_derive_keys( mbedtls_ssl_context *ssl )
{
    int ret = 0;
...
#if defined(MBEDTLS_SSL_PROTO_SSL3)
FLASH_FUNC void ssl_calc_verify_ssl( mbedtls_ssl_context *ssl, unsigned char hash[36] )
{
    mbedtls_md5_context md5;
    mbedtls_sha1_context sha1;
...
#if defined(MBEDTLS_SSL_PROTO_TLS1) || defined(MBEDTLS_SSL_PROTO_TLS1_1)
FLASH_FUNC void ssl_calc_verify_tls( mbedtls_ssl_context *ssl, unsigned char hash[36] )
{
    mbedtls_md5_context md5;
    mbedtls_sha1_context sha1;
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
#if defined(MBEDTLS_SHA256_C)
FLASH_FUNC void ssl_calc_verify_tls_sha256( mbedtls_ssl_context *ssl, unsigned char hash[32] )
{
...
#if defined(MBEDTLS_SHA512_C)
FLASH_FUNC void ssl_calc_verify_tls_sha384( mbedtls_ssl_context *ssl, unsigned char hash[48] )
{
...
FLASH_FUNC int mbedtls_ssl_psk_derive_premaster( mbedtls_ssl_context *ssl, mbedtls_key_exchange_type_t key_ex )
{
...
FLASH_FUNC static void ssl_mac( mbedtls_md_context_t *md_ctx,
 ...
 */
FLASH_FUNC static int ssl_encrypt_buf( mbedtls_ssl_context *ssl )
{
...
FLASH_FUNC static int ssl_decrypt_buf( mbedtls_ssl_context *ssl )
{
    size_t i;
    mbedtls_cipher_mode_t mode;
    int auth_done = 0;
 ...
 */
FLASH_FUNC static int ssl_compress_buf( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC static int ssl_decompress_buf( mbedtls_ssl_context *ssl )
{
    int ret;
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
FLASH_FUNC static int ssl_resend_hello_request( mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC int mbedtls_ssl_fetch_input( mbedtls_ssl_context *ssl, size_t nb_want )
{
    int ret;
    size_t len;
	...
	if(ssl->in.buf == NULL)
		{
		printf("mbedtls_ssl_fetch_input:in NULL\n");
		}
...
//			printf("in_left: %d, nb_want: %d :in\n", ssl->in.left, nb_want);
            ...
            else
            {
                if( ssl->f_recv_timeout != NULL )
                {
              	    /*sometimes timeout, and can not connect correctly in wireless envirionment, here add the timeout time 
                                  when set 0,  is infinate, only add when > 0
					*/
                ...
                {
                 // printf("=> NO timeout time, just read in block mode\n");
 ...
 */
FLASH_FUNC int mbedtls_ssl_flush_output( mbedtls_ssl_context *ssl )
{
    int ret;
    unsigned char *buf, i;
 ...
 */
FLASH_FUNC static int ssl_flight_append( mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC static void ssl_flight_free( mbedtls_ssl_flight_item *flight )
{
 ...
 */
FLASH_FUNC static void ssl_swap_epochs( mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC int mbedtls_ssl_resend( mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC void mbedtls_ssl_recv_flight_completed( mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC void mbedtls_ssl_send_flight_completed( mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC int mbedtls_ssl_write_record( mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC static void ssl_bitmask_set( unsigned char *mask, size_t offset, size_t len )
{
 ...
 */
FLASH_FUNC static int ssl_bitmask_check( unsigned char *mask, size_t len )
{
    size_t i;
 ...
 */
FLASH_FUNC static int ssl_reassemble_dtls_handshake( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->handshake->hs_msg == NULL )
    {
        ...
        MBEDTLS_SSL_DEBUG_MSG( 2, ( "initialize reassembly, total length = %d",
                            msg_len ) );
		//printf("client hello, adding server name extension:in\n");
...
FLASH_FUNC int mbedtls_ssl_prepare_handshake_record( mbedtls_ssl_context *ssl )
{
...
FLASH_FUNC void mbedtls_ssl_update_handshake_status( mbedtls_ssl_context *ssl )
{
 ...
 */
#if defined(MBEDTLS_SSL_DTLS_ANTI_REPLAY)
FLASH_FUNC static void ssl_dtls_replay_reset( mbedtls_ssl_context *ssl )
{
    ssl->in_window_top = 0;
    ssl->in_window = 0;
}

FLASH_FUNC static inline uint64_t ssl_load_six_bytes( unsigned char *buf )
{
 ...
 */
FLASH_FUNC int mbedtls_ssl_dtls_replay_check( mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC void mbedtls_ssl_dtls_replay_update( mbedtls_ssl_context *ssl )
{
    uint64_t rec_seqnum = ssl_load_six_bytes( ssl->in.ctr + 2 );
 ...
 */
FLASH_FUNC static int ssl_check_dtls_clihlo_cookie(
 ...
 * cannot not return 0.
 */
FLASH_FUNC static int ssl_handle_possible_reconnect( mbedtls_ssl_context *ssl )
{
    int ret;
    size_t len;
 ...
 */
FLASH_FUNC static int ssl_parse_record_header( mbedtls_ssl_context *ssl )
{
	...
	//printf("ssl_parse_record_header:in\n");
	...
	//printf("=> ssl_parse_record_header done? \n");
 ...
 */
FLASH_FUNC static int ssl_prepare_record_content( mbedtls_ssl_context *ssl )
{
    int ret, done = 0;
 ...
 *
 */
FLASH_FUNC int mbedtls_ssl_read_record( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC int mbedtls_ssl_read_record_layer( mbedtls_ssl_context *ssl )
{
    int ret;
     ...
     *
     */
	//	printf("=> mbedtls_ssl_read_record_layer...\n");

    /* Case (1): Handshake messages */
    if( ssl->in_hslen != 0 )
    {
    
//	printf("=> xxxmbedtls_ssl_read_record_layer...\n");
	//printf("=> xxxmbedtls_ssl_read_record_layer...\n");
...
read_record_header:
    ...
    if( ( ret = mbedtls_ssl_fetch_input( ssl, mbedtls_ssl_hdr_len( ssl ) ) ) != 0 )
    {
		...
		//printf("=> mbedtls_ssl_fetch_input :%d returned\n", ret);
        return( ret );
    }
...
#endif
		//printf("=> ssl_parse_record_header :%d\n", ret);
        return( ret );
    }
...
FLASH_FUNC int mbedtls_ssl_handle_message_type( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC int mbedtls_ssl_send_fatal_handshake_failure( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC int mbedtls_ssl_send_alert_message( mbedtls_ssl_context *ssl,
                            unsigned char level,
...
FLASH_FUNC int mbedtls_ssl_write_certificate( mbedtls_ssl_context *ssl )
{
...
FLASH_FUNC int mbedtls_ssl_parse_certificate( mbedtls_ssl_context *ssl )
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info = ssl->transform_negotiate->ciphersuite_info;
...
FLASH_FUNC int mbedtls_ssl_write_change_cipher_spec( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC int mbedtls_ssl_parse_change_cipher_spec( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC void mbedtls_ssl_optimize_checksum( mbedtls_ssl_context *ssl,
...
FLASH_FUNC void mbedtls_ssl_reset_checksum( mbedtls_ssl_context *ssl )
{
#if defined(MBEDTLS_SSL_PROTO_SSL3) || defined(MBEDTLS_SSL_PROTO_TLS1) || \
    defined(MBEDTLS_SSL_PROTO_TLS1_1)
...
FLASH_FUNC static void ssl_update_checksum_start( mbedtls_ssl_context *ssl,
                                       const unsigned char *buf, size_t len )
{
#if defined(MBEDTLS_SSL_PROTO_SSL3) || defined(MBEDTLS_SSL_PROTO_TLS1) || \
    defined(MBEDTLS_SSL_PROTO_TLS1_1)
...
#if defined(MBEDTLS_SSL_PROTO_SSL3) || defined(MBEDTLS_SSL_PROTO_TLS1) || \
    defined(MBEDTLS_SSL_PROTO_TLS1_1)
FLASH_FUNC static void ssl_update_checksum_md5sha1( mbedtls_ssl_context *ssl,
                                         const unsigned char *buf, size_t len )
{
     mbedtls_md5_update_ret( &ssl->handshake->fin_md5 , buf, len );
    mbedtls_sha1_update_ret( &ssl->handshake->fin_sha1, buf, len );
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
#if defined(MBEDTLS_SHA256_C)
FLASH_FUNC static void ssl_update_checksum_sha256( mbedtls_ssl_context *ssl,
                                        const unsigned char *buf, size_t len )
{
    mbedtls_sha256_update_ret( &ssl->handshake->fin_sha256, buf, len );
}
...
#if defined(MBEDTLS_SHA512_C)
FLASH_FUNC static void ssl_update_checksum_sha384( mbedtls_ssl_context *ssl,
                                        const unsigned char *buf, size_t len )
{
    mbedtls_sha512_update_ret( &ssl->handshake->fin_sha512, buf, len );
}
...
#if defined(MBEDTLS_SSL_PROTO_SSL3)
FLASH_FUNC static void ssl_calc_finished_ssl(
...
#if defined(MBEDTLS_SSL_PROTO_TLS1) || defined(MBEDTLS_SSL_PROTO_TLS1_1)
FLASH_FUNC static void ssl_calc_finished_tls(
                mbedtls_ssl_context *ssl, unsigned char *buf, int from )
{
    int len = 12;
    const char *sender;
...
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
#if defined(MBEDTLS_SHA256_C)
FLASH_FUNC static void ssl_calc_finished_tls_sha256(
                mbedtls_ssl_context *ssl, unsigned char *buf, int from )
{
    int len = 12;
    const char *sender;
...
#if defined(MBEDTLS_SHA512_C)
FLASH_FUNC static void ssl_calc_finished_tls_sha384(
                mbedtls_ssl_context *ssl, unsigned char *buf, int from )
{
    int len = 12;
    const char *sender;
...
FLASH_FUNC static void ssl_handshake_wrapup_free_hs_transform( mbedtls_ssl_context *ssl )
{
...
FLASH_FUNC void mbedtls_ssl_handshake_wrapup( mbedtls_ssl_context *ssl )
{
...
FLASH_FUNC int mbedtls_ssl_write_finished( mbedtls_ssl_context *ssl )
{
    int ret, hash_len;
...
FLASH_FUNC int mbedtls_ssl_parse_finished( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC static void ssl_handshake_params_init( mbedtls_ssl_handshake_params *handshake )
{
...
FLASH_FUNC static void ssl_transform_init( mbedtls_ssl_transform *transform )
{
...
FLASH_FUNC void mbedtls_ssl_session_init( mbedtls_ssl_session *session )
{
    memset( session, 0, sizeof(mbedtls_ssl_session) );
}

FLASH_FUNC static int ssl_handshake_init( mbedtls_ssl_context *ssl )
{
...
FLASH_FUNC static int ssl_cookie_write_dummy( void *ctx,
...
FLASH_FUNC static int ssl_cookie_check_dummy( void *ctx,
 ...
 */
FLASH_FUNC void mbedtls_ssl_init( mbedtls_ssl_context *ssl )
{
                                  ...
                                  size_t max_content_len )
{
    ...
    (void)ssl;
	 ...
	 //add by eason@altobeam for colink, rec->max_content_len :2810 ,default max_content_len:2380+       
	 max_content_len += 512;
	...
	//printf("mbedtls_ssl_alloc_record_buf,will alloc! len:%d max:%d\n", len, max_content_len);
	...
	//printf("mbedtls_ssl_alloc_record_buf set  rec->max_content_len :%d  \r\n", len, min); 
 ...
 */
FLASH_FUNC int mbedtls_ssl_setup( mbedtls_ssl_context *ssl,
                                     ...
                                     size_t content_len )
{
  ...
  //   printf("mbedtls_ssl_confirm_content_len, content_len:%d max:%d\r\n", content_len, rec->max_content_len);
    if( rec->buf &&
        content_len <= (size_t)rec->max_content_len )
    	{

	//	printf("VVVVVVVV memory enough return \n");
	    return( 0 );
    	}
 ...
 */
FLASH_FUNC static int ssl_session_reset_int( mbedtls_ssl_context *ssl, int partial )
{
    int ret;
 ...
 * all application-set variables, function pointers and data.
 */
FLASH_FUNC int mbedtls_ssl_session_reset( mbedtls_ssl_context *ssl )
{
 ...
 * SSL set accessors
 */
FLASH_FUNC void mbedtls_ssl_conf_endpoint( mbedtls_ssl_config *conf, int endpoint )
{
    conf->endpoint   = endpoint;
}

FLASH_FUNC void mbedtls_ssl_conf_transport( mbedtls_ssl_config *conf, int transport )
{
    conf->transport = transport;
}

#if defined(MBEDTLS_SSL_DTLS_ANTI_REPLAY)
FLASH_FUNC void mbedtls_ssl_conf_dtls_anti_replay( mbedtls_ssl_config *conf, char mode )
{
...
#if defined(MBEDTLS_SSL_DTLS_BADMAC_LIMIT)
FLASH_FUNC void mbedtls_ssl_conf_dtls_badmac_limit( mbedtls_ssl_config *conf, unsigned limit )
{
...
#if defined(MBEDTLS_SSL_PROTO_DTLS)
FLASH_FUNC void mbedtls_ssl_conf_handshake_timeout( mbedtls_ssl_config *conf, uint32_t min, uint32_t max )
{
...
FLASH_FUNC void mbedtls_ssl_conf_authmode( mbedtls_ssl_config *conf, int authmode )
{
    conf->authmode   = authmode;
}

#if defined(MBEDTLS_X509_CRT_PARSE_C)
FLASH_FUNC void mbedtls_ssl_conf_verify( mbedtls_ssl_config *conf,
...
FLASH_FUNC void mbedtls_ssl_conf_rng( mbedtls_ssl_config *conf,
...
FLASH_FUNC void mbedtls_ssl_conf_dbg( mbedtls_ssl_config *conf,
...
FLASH_FUNC void mbedtls_ssl_set_bio( mbedtls_ssl_context *ssl,
        void *p_bio,
...
FLASH_FUNC void mbedtls_ssl_conf_read_timeout( mbedtls_ssl_config *conf, uint32_t timeout )
{
    conf->read_timeout   = timeout;
}

FLASH_FUNC void mbedtls_ssl_set_timer_cb( mbedtls_ssl_context *ssl,
                               void *p_timer,
...
#if defined(MBEDTLS_SSL_SRV_C)
FLASH_FUNC void mbedtls_ssl_conf_session_cache( mbedtls_ssl_config *conf,
        void *p_cache,
...
#if defined(MBEDTLS_SSL_CLI_C)
FLASH_FUNC int mbedtls_ssl_set_session( mbedtls_ssl_context *ssl, const mbedtls_ssl_session *session )
{
    int ret;
...
FLASH_FUNC void mbedtls_ssl_conf_ciphersuites( mbedtls_ssl_config *conf,
...
FLASH_FUNC void mbedtls_ssl_conf_ciphersuites_for_version( mbedtls_ssl_config *conf,
                                       const int *ciphersuites,
                                       int major, int minor )
{
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
FLASH_FUNC void mbedtls_ssl_conf_cert_profile( mbedtls_ssl_config *conf,
...
FLASH_FUNC static int ssl_append_key_cert( mbedtls_ssl_key_cert **head,
...
FLASH_FUNC int mbedtls_ssl_conf_own_cert( mbedtls_ssl_config *conf,
...
FLASH_FUNC void mbedtls_ssl_conf_ca_chain( mbedtls_ssl_config *conf,
                               mbedtls_x509_crt *ca_chain,
                               mbedtls_x509_crl *ca_crl )
{
...
#if defined(MBEDTLS_SSL_SERVER_NAME_INDICATION)
FLASH_FUNC int mbedtls_ssl_set_hs_own_cert( mbedtls_ssl_context *ssl,
                                 mbedtls_x509_crt *own_cert,
                                 mbedtls_pk_context *pk_key )
{
...
FLASH_FUNC void mbedtls_ssl_set_hs_ca_chain( mbedtls_ssl_context *ssl,
                                  mbedtls_x509_crt *ca_chain,
                                  mbedtls_x509_crl *ca_crl )
{
...
FLASH_FUNC void mbedtls_ssl_set_hs_authmode( mbedtls_ssl_context *ssl,
                                  int authmode )
{
 ...
 */
FLASH_FUNC int mbedtls_ssl_set_hs_ecjpake_password( mbedtls_ssl_context *ssl,
...
#if defined(MBEDTLS_KEY_EXCHANGE__SOME__PSK_ENABLED)
FLASH_FUNC int mbedtls_ssl_conf_psk( mbedtls_ssl_config *conf,
...
FLASH_FUNC int mbedtls_ssl_set_hs_psk( mbedtls_ssl_context *ssl,
                            const unsigned char *psk, size_t psk_len )
{
...
FLASH_FUNC void mbedtls_ssl_conf_psk_cb( mbedtls_ssl_config *conf,
...
FLASH_FUNC int mbedtls_ssl_conf_dh_param( mbedtls_ssl_config *conf, const char *dhm_P, const char *dhm_G )
{
    int ret;
...
FLASH_FUNC int mbedtls_ssl_conf_dh_param_bin( mbedtls_ssl_config *conf,
...
FLASH_FUNC int mbedtls_ssl_conf_dh_param_ctx( mbedtls_ssl_config *conf, mbedtls_dhm_context *dhm_ctx )
{
    int ret;
 ...
 */
FLASH_FUNC void mbedtls_ssl_conf_dhm_min_bitlen( mbedtls_ssl_config *conf,
                                      unsigned int bitlen )
{
 ...
 */
FLASH_FUNC void mbedtls_ssl_conf_sig_hashes( mbedtls_ssl_config *conf,
                                  const int *hashes )
{
 ...
 */
FLASH_FUNC void mbedtls_ssl_conf_curves( mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
FLASH_FUNC int mbedtls_ssl_set_hostname( mbedtls_ssl_context *ssl, const char *hostname )
{
...
#if defined(MBEDTLS_SSL_SERVER_NAME_INDICATION)
FLASH_FUNC void mbedtls_ssl_conf_sni( mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_SSL_ALPN)
FLASH_FUNC int mbedtls_ssl_conf_alpn_protocols( mbedtls_ssl_config *conf, const char **protos )
{
...
FLASH_FUNC const char *mbedtls_ssl_get_alpn_protocol( const mbedtls_ssl_context *ssl )
{
    return( ssl->alpn_chosen );
}
#endif /* MBEDTLS_SSL_ALPN */

FLASH_FUNC void mbedtls_ssl_conf_max_version( mbedtls_ssl_config *conf, int major, int minor )
{
    conf->max_major_ver = major;
    conf->max_minor_ver = minor;
}

FLASH_FUNC void mbedtls_ssl_conf_min_version( mbedtls_ssl_config *conf, int major, int minor )
{
...
FLASH_FUNC void mbedtls_ssl_conf_fallback( mbedtls_ssl_config *conf, char fallback )
{
...
#if defined(MBEDTLS_SSL_SRV_C)
FLASH_FUNC void mbedtls_ssl_conf_cert_req_ca_list( mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_SSL_ENCRYPT_THEN_MAC)
FLASH_FUNC void mbedtls_ssl_conf_encrypt_then_mac( mbedtls_ssl_config *conf, char etm )
{
...
#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
FLASH_FUNC void mbedtls_ssl_conf_extended_master_secret( mbedtls_ssl_config *conf, char ems )
{
...
FLASH_FUNC void mbedtls_ssl_conf_arc4_support( mbedtls_ssl_config *conf, char arc4 )
{
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
FLASH_FUNC int mbedtls_ssl_conf_max_frag_len( mbedtls_ssl_config *conf, unsigned char mfl_code )
{
...
#if defined(MBEDTLS_SSL_TRUNCATED_HMAC)
FLASH_FUNC void mbedtls_ssl_conf_truncated_hmac( mbedtls_ssl_config *conf, int truncate )
{
...
#if defined(MBEDTLS_SSL_CBC_RECORD_SPLITTING)
FLASH_FUNC void mbedtls_ssl_conf_cbc_record_splitting( mbedtls_ssl_config *conf, char split )
{
    conf->cbc_record_splitting = split;
}
#endif

FLASH_FUNC void mbedtls_ssl_conf_legacy_renegotiation( mbedtls_ssl_config *conf, int allow_legacy )
{
    conf->allow_legacy_renegotiation = allow_legacy;
}

#if defined(MBEDTLS_SSL_RENEGOTIATION)
FLASH_FUNC void mbedtls_ssl_conf_renegotiation( mbedtls_ssl_config *conf, int renegotiation )
{
    conf->disable_renegotiation = renegotiation;
}

FLASH_FUNC void mbedtls_ssl_conf_renegotiation_enforced( mbedtls_ssl_config *conf, int max_records )
{
    conf->renego_max_records = max_records;
}

FLASH_FUNC void mbedtls_ssl_conf_renegotiation_period( mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_SSL_CLI_C)
FLASH_FUNC void mbedtls_ssl_conf_session_tickets( mbedtls_ssl_config *conf, int use_tickets )
{
...
#if defined(MBEDTLS_SSL_SRV_C)
FLASH_FUNC void mbedtls_ssl_conf_session_tickets_cb( mbedtls_ssl_config *conf,
...
#if defined(MBEDTLS_SSL_EXPORT_KEYS)
vvoid mbedtls_ssl_conf_export_keys_cb( mbedtls_ssl_config *conf,
 ...
 * SSL get accessors
 */
FLASH_FUNC size_t mbedtls_ssl_get_bytes_avail( const mbedtls_ssl_context *ssl )
{
    return( ssl->in.offt == NULL ? 0 : ssl->in.msglen );
}

FLASH_FUNC uint32_t mbedtls_ssl_get_verify_result( const mbedtls_ssl_context *ssl )
{
    if( ssl->session != NULL )
...
FLASH_FUNC const char *mbedtls_ssl_get_ciphersuite( const mbedtls_ssl_context *ssl )
{
...
FLASH_FUNC const char *mbedtls_ssl_get_version( const mbedtls_ssl_context *ssl )
{
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if( ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
    {
...
FLASH_FUNC int mbedtls_ssl_get_record_expansion( const mbedtls_ssl_context *ssl )
{
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
FLASH_FUNC size_t mbedtls_ssl_get_max_frag_len( const mbedtls_ssl_context *ssl )
{
    size_t max_len;
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
FLASH_FUNC const mbedtls_x509_crt *mbedtls_ssl_get_peer_cert( const mbedtls_ssl_context *ssl )
{
    if( ssl == NULL || ssl->session == NULL )
        return( NULL );
...
#if defined(MBEDTLS_SSL_CLI_C)
FLASH_FUNC int mbedtls_ssl_get_session( const mbedtls_ssl_context *ssl, mbedtls_ssl_session *dst )
{
    if( ssl == NULL ||
        dst == NULL ||
        ssl->session == NULL ||
 ...
 */
FLASH_FUNC int mbedtls_ssl_handshake_step( mbedtls_ssl_context *ssl )
{
    int ret = MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;
 ...
 */
FLASH_FUNC int mbedtls_ssl_handshake( mbedtls_ssl_context *ssl )
{
    int ret = 0;
 ...
 */
FLASH_FUNC static int ssl_write_hello_request( mbedtls_ssl_context *ssl )
{
    int ret;
 ...
 */
FLASH_FUNC static int ssl_start_renegotiation( mbedtls_ssl_context *ssl )
{
    int ret;
 ...
 */
FLASH_FUNC int mbedtls_ssl_renegotiate( mbedtls_ssl_context *ssl )
{
    int ret = MBEDTLS_ERR_SSL_FEATURE_UNAVAILABLE;
 ...
 */
FLASH_FUNC static int ssl_check_ctr_renegotiate( mbedtls_ssl_context *ssl )
{
 ...
 */
FLASH_FUNC int mbedtls_ssl_read( mbedtls_ssl_context *ssl, unsigned char *buf, size_t len )
{
    int ret;
    size_t n;
 ...
 */
FLASH_FUNC static int ssl_write_real( mbedtls_ssl_context *ssl,
                           const unsigned char *buf, size_t len )
{
    int ret;
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
 ...
 */
#if defined(MBEDTLS_SSL_CBC_RECORD_SPLITTING)
FLASH_FUNC static int ssl_write_split( mbedtls_ssl_context *ssl,
                            const unsigned char *buf, size_t len )
{
    int ret;
 ...
 */
FLASH_FUNC int mbedtls_ssl_write( mbedtls_ssl_context *ssl, const unsigned char *buf, size_t len )
{
    int ret;
 ...
 */
FLASH_FUNC int mbedtls_ssl_close_notify( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC void mbedtls_ssl_transform_free( mbedtls_ssl_transform *transform )
{
    if( transform == NULL )
        return;
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
FLASH_FUNC static void ssl_key_cert_free( mbedtls_ssl_key_cert *key_cert )
{
...
FLASH_FUNC void mbedtls_ssl_handshake_free( mbedtls_ssl_handshake_params *handshake )
{
    if( handshake == NULL )
        return;
...
FLASH_FUNC void mbedtls_ssl_session_free( mbedtls_ssl_session *session )
{
    if( session == NULL )
        return;
 ...
 * Free an SSL context
 */
FLASH_FUNC void mbedtls_ssl_free( mbedtls_ssl_context *ssl )
{
    if( ssl == NULL )
        return;
 ...
 */
FLASH_FUNC void mbedtls_ssl_config_init( mbedtls_ssl_config *conf )
{
 ...
 */
FLASH_FUNC int mbedtls_ssl_config_defaults( mbedtls_ssl_config *conf,
 ...
 */
FLASH_FUNC void mbedtls_ssl_config_free( mbedtls_ssl_config *conf )
{
#if defined(MBEDTLS_DHM_C)
    mbedtls_mpi_free( &conf->dhm_P );
    mbedtls_mpi_free( &conf->dhm_G );
 ...
 */
FLASH_FUNC unsigned char mbedtls_ssl_sig_from_pk( mbedtls_pk_context *pk )
{
...
FLASH_FUNC unsigned char mbedtls_ssl_sig_from_pk_alg( mbedtls_pk_type_t type )
{
    switch( type ) {
        case MBEDTLS_PK_RSA:
            return( MBEDTLS_SSL_SIG_RSA );
...
FLASH_FUNC mbedtls_pk_type_t mbedtls_ssl_pk_alg_from_sig( unsigned char sig )
{
    switch( sig )
    {
#if defined(MBEDTLS_RSA_C)
...
FLASH_FUNC mbedtls_md_type_t mbedtls_ssl_sig_hash_set_find( mbedtls_ssl_sig_hash_set_t *set,
...
FLASH_FUNC void mbedtls_ssl_sig_hash_set_add( mbedtls_ssl_sig_hash_set_t *set,
                                   mbedtls_pk_type_t sig_alg,
...
FLASH_FUNC void mbedtls_ssl_sig_hash_set_const_hash( mbedtls_ssl_sig_hash_set_t *set,
                                          mbedtls_md_type_t md_alg )
{
    set->rsa   = md_alg;
    set->ecdsa = md_alg;
 ...
 */
FLASH_FUNC mbedtls_md_type_t mbedtls_ssl_md_alg_from_hash( unsigned char hash )
{
    switch( hash )
    {
 ...
 */
FLASH_FUNC unsigned char mbedtls_ssl_hash_from_md_alg( int md )
{
    switch( md )
    {
#if defined(MBEDTLS_MD5_C)
 ...
 */
FLASH_FUNC int mbedtls_ssl_check_curve( const mbedtls_ssl_context *ssl, mbedtls_ecp_group_id grp_id )
{
 ...
 * Return 0 if we're willing to use it, -1 otherwise.
 */
FLASH_FUNC int mbedtls_ssl_check_sig_hash( const mbedtls_ssl_context *ssl,
...
#if defined(MBEDTLS_X509_CRT_PARSE_C)
FLASH_FUNC int mbedtls_ssl_check_cert_usage( const mbedtls_x509_crt *cert,
 ...
 */
FLASH_FUNC void mbedtls_ssl_write_version( int major, int minor, int transport,
                        unsigned char ver[2] )
{
#if defined(MBEDTLS_SSL_PROTO_DTLS)
    if( transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM )
...
FLASH_FUNC void mbedtls_ssl_read_version( int *major, int *minor, int transport,
...
FLASH_FUNC int mbedtls_ssl_set_calc_verify_md( mbedtls_ssl_context *ssl, int md )
{
#if defined(MBEDTLS_SSL_PROTO_TLS1_2)
    if( ssl->minor_ver != MBEDTLS_SSL_MINOR_VERSION_3 )
...
#if defined(MBEDTLS_SSL_PROTO_SSL3) || defined(MBEDTLS_SSL_PROTO_TLS1) || \
    defined(MBEDTLS_SSL_PROTO_TLS1_1)
FLASH_FUNC int mbedtls_ssl_get_key_exchange_md_ssl_tls( mbedtls_ssl_context *ssl,
...
#if defined(MBEDTLS_SSL_PROTO_TLS1) || defined(MBEDTLS_SSL_PROTO_TLS1_1) || \
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
FLASH_FUNC int mbedtls_ssl_get_key_exchange_md_tls1_2( mbedtls_ssl_context *ssl,
                                       unsigned char *output,
                                       unsigned char *data, size_t data_len,
                                       mbedtls_md_type_t md_alg )
{
