 ...
 */
FLASH_RDATA static const int ciphersuite_preference[] =
{
...
FLASH_RDATA static const mbedtls_ssl_ciphersuite_t ciphersuite_definitions[] =
{
...
#else
#include "mbedtls/mbedtls_platform.h"
static int *supported_ciphersuites = NULL;
#endif
static int supported_init = 0;

FLASH_FUNC const int *mbedtls_ssl_list_ciphersuites( void )
{
    /*
     * On initial call filter out all ciphersuites not supported by current
     * build based on presence in the ciphersuite_definitions.
     */
     if(NULL == supported_ciphersuites)
     {
        supported_ciphersuites = (int *)mbedtls_calloc(1,MAX_CIPHERSUITES*sizeof(int));
     }
    if( supported_init == 0 )
    {
        const int *p;
...
FLASH_FUNC const mbedtls_ssl_ciphersuite_t *mbedtls_ssl_ciphersuite_from_string(
...
FLASH_FUNC const mbedtls_ssl_ciphersuite_t *mbedtls_ssl_ciphersuite_from_id( int ciphersuite )
{
    const mbedtls_ssl_ciphersuite_t *cur = ciphersuite_definitions;
...
FLASH_FUNC const char *mbedtls_ssl_get_ciphersuite_name( const int ciphersuite_id )
{
...
FLASH_FUNC int mbedtls_ssl_get_ciphersuite_id( const char *ciphersuite_name )
{
    const mbedtls_ssl_ciphersuite_t *cur;
...
FLASH_FUNC mbedtls_pk_type_t mbedtls_ssl_get_ciphersuite_sig_pk_alg( const mbedtls_ssl_ciphersuite_t *info )
{
...
FLASH_FUNC mbedtls_pk_type_t mbedtls_ssl_get_ciphersuite_sig_alg( const mbedtls_ssl_ciphersuite_t *info )
{
    switch( info->key_exchange )
    {
        case MBEDTLS_KEY_EXCHANGE_RSA:
...
FLASH_FUNC int mbedtls_ssl_ciphersuite_uses_ec( const mbedtls_ssl_ciphersuite_t *info )
{
    switch( info->key_exchange )
    {
        case MBEDTLS_KEY_EXCHANGE_ECDHE_RSA:
...
FLASH_FUNC int mbedtls_ssl_ciphersuite_uses_psk( const mbedtls_ssl_ciphersuite_t *info )
{
    switch( info->key_exchange )
    {
