...
typedef struct{
	uint8 joylink_start_flag;
	struct system_timer *joylink_cyclecall_timer;
	struct system_timer * joylink_expire_timer;
}joylink_smnt_priv_t;
...
extern uint8 secretKey[17];
extern int joylink_start(joylink_smnt_param_t *joy_init);
extern int joylink_stop();
/*if have been start return 1, not start return 0*/
extern int joylink_is_start();

extern void joylink_smnt_init(joylink_smnt_param_t param);                                  
extern int  joylink_smnt_cyclecall(void);                                               
extern void joylink_smnt_datahandler(PHEADER_802_11 pHeader, int length);               
extern void joylink_smnt_release(void);
extern void joylink_smnt_reset(void);

extern int joy_switch_channel_cb(uint8 channelnum);
extern int get_result_cb(joylink_smnt_result_t result);
