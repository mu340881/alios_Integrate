...
static inline void GIE_SAVE(unsigned int *var)
{
	*var = GET_PSW();
	GIE_DISABLE();
}

static inline void GIE_RESTORE(unsigned int var)
{
	if (var & PSW_mskGIE)
		GIE_ENABLE();
}
 ...
 * SYSTEM CONSTANTS
 ***********************************/
#define VECTOR_BASE             EILM_BASE

#define IRQ_STACK_SIZE		CONFIG_IRQ_STACK_SIZE	/* Define the IRQ stack size */
