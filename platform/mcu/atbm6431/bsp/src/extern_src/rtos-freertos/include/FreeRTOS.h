...
#if ( configGENERATE_RUN_TIME_STATS == 1 ) //add by wp
	#define portCONFIGURE_TIMER_FOR_RUN_TIME_STATS() //we will init time at APB_CNT_Init,so not need define function here
extern unsigned int  HAL_TIME_US();
	...
	extern void vApplicationLightSleep( TickType_t xExpectedIdleTime );
	#define portSUPPRESS_TICKS_AND_SLEEP( xExpectedIdleTime )  vApplicationLightSleep(xExpectedIdleTime) 
#endif
