...
#if 0
#include <stdio.h>
#define MBEDTLS_SSL_DEBUG_MSG( level, args )   do{iot_printf args;iot_printf("\n"); }while(0)
#define MBEDTLS_SSL_DEBUG_RET( level, text, ret )  do{iot_printf( text );iot_printf("return:%d\n",ret); }while(0)
#endif
...
FLASH_FUNC int mbedtls_ssl_set_client_transport_id( mbedtls_ssl_context *ssl,
...
FLASH_FUNC void mbedtls_ssl_conf_dtls_cookies( mbedtls_ssl_config *conf,
...
FLASH_FUNC static int ssl_parse_servername_ext( mbedtls_ssl_context *ssl,
...
FLASH_FUNC static int ssl_parse_renegotiation_info( mbedtls_ssl_context *ssl,
                                         const unsigned char *buf,
                                         size_t len )
{
 ...
 *
 */
FLASH_FUNC static int ssl_parse_signature_algorithms_ext( mbedtls_ssl_context *ssl,
                                               const unsigned char *buf,
                                               size_t len )
{
...
FLASH_FUNC static int ssl_parse_supported_elliptic_curves( mbedtls_ssl_context *ssl,
                                                const unsigned char *buf,
                                                size_t len )
{
...
FLASH_FUNC static int ssl_parse_supported_point_formats( mbedtls_ssl_context *ssl,
                                              const unsigned char *buf,
                                              size_t len )
{
    size_t list_size;
...
#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
FLASH_FUNC static int ssl_parse_ecjpake_kkpp( mbedtls_ssl_context *ssl,
                                   const unsigned char *buf,
                                   size_t len )
{
    int ret;
...
FLASH_FUNC static int ssl_parse_max_fragment_length_ext( mbedtls_ssl_context *ssl,
                                              const unsigned char *buf,
                                              size_t len )
{
...
FLASH_FUNC static int ssl_parse_truncated_hmac_ext( mbedtls_ssl_context *ssl,
                                         const unsigned char *buf,
                                         size_t len )
{
    if( len != 0 )
...
FLASH_FUNC static int ssl_parse_encrypt_then_mac_ext( mbedtls_ssl_context *ssl,
                                      const unsigned char *buf,
                                      size_t len )
{
    if( len != 0 )
...
FLASH_FUNC static int ssl_parse_extended_ms_ext( mbedtls_ssl_context *ssl,
                                      const unsigned char *buf,
                                      size_t len )
{
    if( len != 0 )
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
FLASH_FUNC static int ssl_parse_session_ticket_ext( mbedtls_ssl_context *ssl,
                                         unsigned char *buf,
                                         size_t len )
{
    int ret;
...
FLASH_FUNC static int ssl_parse_alpn_ext( mbedtls_ssl_context *ssl,
...
FLASH_FUNC static int ssl_check_key_curve( mbedtls_pk_context *pk,
 ...
 */
FLASH_FUNC static int ssl_pick_cert( mbedtls_ssl_context *ssl,
 ...
 */
FLASH_FUNC static int ssl_ciphersuite_match( mbedtls_ssl_context *ssl, int suite_id,
                                  const mbedtls_ssl_ciphersuite_t **ciphersuite_info )
{
...
FLASH_FUNC static int ssl_parse_client_hello_v2( mbedtls_ssl_context *ssl )
{
...
FLASH_FUNC static int ssl_parse_client_hello( mbedtls_ssl_context *ssl )
{
    int ret, got_common_suite;
    size_t i, j;
...
#if defined(MBEDTLS_SSL_TRUNCATED_HMAC)
FLASH_FUNC static void ssl_write_truncated_hmac_ext( mbedtls_ssl_context *ssl,
                                          unsigned char *buf,
                                          size_t *olen )
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_ENCRYPT_THEN_MAC)
FLASH_FUNC static void ssl_write_encrypt_then_mac_ext( mbedtls_ssl_context *ssl,
                                            unsigned char *buf,
                                            size_t *olen )
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
FLASH_FUNC static void ssl_write_extended_ms_ext( mbedtls_ssl_context *ssl,
                                       unsigned char *buf,
                                       size_t *olen )
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
FLASH_FUNC static void ssl_write_session_ticket_ext( mbedtls_ssl_context *ssl,
                                          unsigned char *buf,
                                          size_t *olen )
{
    unsigned char *p = buf;
...
FLASH_FUNC static void ssl_write_renegotiation_ext( mbedtls_ssl_context *ssl,
                                         unsigned char *buf,
                                         size_t *olen )
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
FLASH_FUNC static void ssl_write_max_fragment_length_ext( mbedtls_ssl_context *ssl,
                                               unsigned char *buf,
                                               size_t *olen )
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_ECDH_C) || defined(MBEDTLS_ECDSA_C) || \
    defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
FLASH_FUNC static void ssl_write_supported_point_formats_ext( mbedtls_ssl_context *ssl,
                                                   unsigned char *buf,
                                                   size_t *olen )
{
    unsigned char *p = buf;
...
#if defined(MBEDTLS_KEY_EXCHANGE_ECJPAKE_ENABLED)
FLASH_FUNC static void ssl_write_ecjpake_kkpp_ext( mbedtls_ssl_context *ssl,
                                        unsigned char *buf,
                                        size_t *olen )
{
    int ret;
...
#if defined(MBEDTLS_SSL_ALPN )
FLASH_FUNC static void ssl_write_alpn_ext( mbedtls_ssl_context *ssl,
...
#if defined(MBEDTLS_SSL_DTLS_HELLO_VERIFY)
FLASH_FUNC static int ssl_write_hello_verify_request( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC static int ssl_write_server_hello( mbedtls_ssl_context *ssl )
{
#if defined(MBEDTLS_HAVE_TIME)
    mbedtls_time_t t;
#endif
...
FLASH_FUNC static int ssl_write_certificate_request( mbedtls_ssl_context *ssl )
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
...
FLASH_FUNC static int ssl_write_certificate_request( mbedtls_ssl_context *ssl )
{
    ...
    defined(MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED)
FLASH_FUNC static int ssl_get_ecdh_params_from_cert( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC static int ssl_write_server_key_exchange( mbedtls_ssl_context *ssl )
{
    int ret;
    size_t n = 0;
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
...
FLASH_FUNC static int ssl_write_server_hello_done( mbedtls_ssl_context *ssl )
{
    int ret;
...
FLASH_FUNC static int ssl_parse_client_dh_public( mbedtls_ssl_context *ssl, unsigned char **p,
...
FLASH_FUNC static int ssl_parse_encrypted_pms( mbedtls_ssl_context *ssl,
                                    const unsigned char *p,
                                    const unsigned char *end,
                                    size_t pms_offset )
{
...
#if defined(MBEDTLS_KEY_EXCHANGE__SOME__PSK_ENABLED)
FLASH_FUNC static int ssl_parse_client_psk_identity( mbedtls_ssl_context *ssl, unsigned char **p,
                                          const unsigned char *end )
{
    int ret = 0;
    size_t n;
...
FLASH_FUNC static int ssl_parse_client_key_exchange( mbedtls_ssl_context *ssl )
{
    int ret;
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info;
    unsigned char *p, *end;
    ...
    MBEDTLS_SSL_DEBUG_MSG( 2, ( "read_record over" ) );
    ...
    !defined(MBEDTLS_KEY_EXCHANGE_ECDH_ECDSA_ENABLED)&& \
    !defined(MBEDTLS_KEY_EXCHANGE_ECDHE_ECDSA_ENABLED)
FLASH_FUNC static int ssl_parse_certificate_verify( mbedtls_ssl_context *ssl )
{
    const mbedtls_ssl_ciphersuite_t *ciphersuite_info =
        ssl->transform_negotiate->ciphersuite_info;
...
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
FLASH_FUNC static int ssl_write_new_session_ticket( mbedtls_ssl_context *ssl )
{
    int ret;
    size_t tlen;
    uint32_t lifetime;
 ...
 */
FLASH_FUNC int mbedtls_ssl_handshake_server_step( mbedtls_ssl_context *ssl )
{
    int ret = 0;
